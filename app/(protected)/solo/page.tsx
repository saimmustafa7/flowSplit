...
import { Plus, Wallet, Trash2, ChevronLeft, ChevronRight } from 'lucide-react'
...
function isSameMonth(expenseDate: string, ym: string): boolean {
  return expenseDate.slice(0, 7) === ym
}

export default function SoloPage() {
  const searchParams = useSearchParams()

  // Determine initial month: from URL ?month=YYYY-MM or current month
  const initialMonth = searchParams.get('month') ?? getYearMonth(new Date())
  const currentYM = getYearMonth(new Date())

  const [selectedMonth, setSelectedMonth] = useState(initialMonth)
  const [allExpenses, setAllExpenses] = useState<SoloExpense[]>([])
  ...
  const expenses = allExpenses.filter(e => isSameMonth(e.expense_date, selectedMonth))

  // Total for selected month
  const monthTotal = expenses.reduce((sum, e) => sum + e.amount, 0)

  // Check if selected month is current or in the future
  const isCurrentOrFuture = selectedMonth >= currentYM
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAllExpenses(prev => [newExp, ...prev].sort((a, b) =>
        b.expense_date.localeCompare(a.expense_date)
      ))
      setIsAdding(false)
      setTitle('')
      setAmountInput('')
      // If the added expense is for a different month, switch to that month
      const addedMonth = date.slice(0, 7)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setAllExpenses(prev => prev.filter(e => e.id !== deleteId))
  ...
  return (
    <div className="px-4 md:px-0 space-y-6">
      ...
      <div className="flex items-center justify-between bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl px-4 py-3">
        <button
          onClick={() => setSelectedMonth(prev => stepMonth(prev, -1))}
          className="p-1 rounded-lg hover:bg-[var(--bg-elevated)] transition-colors text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
        ...
        <div className="text-center">
          <p className="font-bold font-clash text-[var(--text-primary)]">
        ...
        <button
          onClick={() => canGoNext && setSelectedMonth(prev => stepMonth(prev, 1))}
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-strong)] rounded-2xl px-5 py-4 flex items-center justify-between">
        <div>
          <p className="text-sm text-[var(--text-secondary)]">
            {selectedMonth === currentYM ? 'Spent this month' : `Total — ${formatYearMonth(selectedMonth)}`}
          </p>
          <p className="text-3xl font-bold font-clash mt-0.5">{formatCurrency(monthTotal)}</p>
        </div>
        <div className="p-3 bg-[var(--text-primary)] rounded-xl text-[var(--text-inverse)]">
          <Wallet size={22} />
        </div>
      </div>
      ...
      {expenses.length === 0 ? (
        <EmptyState
          icon={Wallet}
          title={`No expenses in ${formatYearMonth(selectedMonth)}`}
          description={
            isCurrentOrFuture
              ? 'Track your personal spending privately.'
              : 'You had no recorded expenses this month.'
          }
          action={
            isCurrentOrFuture ? (
              <Button onClick={() => setIsAdding(true)}>Add your first expense</Button>
            ) : undefined
          }
        />
      ) : (
